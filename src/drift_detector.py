...
def chi_square_test(train_series, test_series):
    """
    Perform Chi-Square test to check if categorical distributions differ.
...
def classify_psi(psi):
    """
    Convert PSI value into drift severity label.
    """
...
def detect_categorical_drift(train_df, prod_df, categorical_cols):
    """
    Detect drift across multiple categorical features.
